...
### 3.8 table
**Additions (2026-09):** static-table cells ARE interpolated (`{patient.fullName}` etc.) in both renderers; `el.hideEmptyRows` drops rows whose value columns (`column.valueColumn`, else every column but the first) are all empty; `el.maxRows` truncates the rows. Editor raw mode shows static rows as typed. `column.group` → a spanning header row above the column headers (ungrouped columns and № span both rows); `column.fillIfSet` → cell background when the cell text is non-empty; `el.numberHeader`/`el.numberWidth` configure the № column (defaults '№' / 28px). `el.nowrap` → cell text is never wrapped (single line, clipped to the cell); rows stay at `rowHeight` unless the font is taller (no vertical padding; header rows still wrap). Header cells are aligned by `headerStyle.align` when it is center/right, else by the column `align`.
